use super::{FromCuda, ZludaObject, module, LiveCheck};
use cuda_types::cuda::*;
use hip_runtime_sys::*;
...
impl ContextStack {
    thread_local! {
        static _stack: RefCell<Vec<(CUcontext, hipDevice_t)>> = RefCell::new(Vec::new());
    }
    pub fn push(handle: CUcontext, device: hipDevice_t) {
        Self::_stack.with(|stack| stack.borrow_mut().push((handle, device)))
    }
    pub fn pop() -> Option<(CUcontext, hipDevice_t)> {
        Self::_stack.with(|stack| stack.borrow_mut().pop())
    }
    pub fn peek() -> Option<(CUcontext, hipDevice_t)> {
        Self::_stack.with(|stack| stack.borrow().last().copied())
    }
}

pub(crate) enum ContextKind {
    Primary,   // cuDevicePrimaryCtx* API
    Secondary, // cuCtx* API
}
...
impl Context {
    pub(crate) fn new(device: hipDevice_t, flags: u32, kind: ContextKind) -> Self {
        Self {
            device: device,
    ...
    pub(crate) fn set_flags(&self, flags: u32) -> () {
        self.flags.store(flags, std::sync::atomic::Ordering::SeqCst);
    }
...
impl ZludaObject for Context {
    ...
    const NullHandle: CUcontext = CUcontext(std::ptr::null_mut());
    const LIVENESS_FAIL: CUerror = CUerror::INVALID_CONTEXT;
...
//    match ContextStack::peek() {
//        Some((hctx, _)) => Ok(hctx),
//        None => Err(CUerror::INVALID_CONTEXT)
//    }
//    
...
pub(crate) unsafe fn create(
    ...
    let ctx = Box::new(LiveCheck::new(Context::new(dev, flags, ContextKind::Secondary)));
    let hctx = ctx.as_handle();
    ContextStack::push(hctx, dev);
    *pctx = hctx;
    Ok(())
}
...
pub(crate) unsafe fn create_v2(
    ...
    create(pctx, flags, dev)
}

pub(crate) unsafe fn destroy(ctx: CUcontext) -> CUresult {
    Ok(())
}
...
pub(crate) unsafe fn set_flags(flags: u32) -> CUresult {
    Ok(())
}

pub(crate) unsafe fn get_stream_priority_range(
    lower: *mut ::std::os::raw::c_int,
...
pub(crate) fn set_current(handle: Option<CUcontext>) -> CUresult {
...
pub(crate) fn get_current(pctx: &mut CUcontext) -> CUresult {
    match ContextStack::peek() {
        Some((hctx, _)) => *pctx = hctx,
        None => *pctx = Context::NullHandle
    }
    Ok(())
}
...
pub(crate) unsafe fn set_cache_config(config: CUfunc_cache) -> CUresult {
    Ok(())
}